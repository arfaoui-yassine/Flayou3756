...
import type { AgentConfig, StockLowEvent } from '@datasouk/shared';
...
export function startCollector(getConfig: () => AgentConfig | null): void {
  stopCollector();
  const tick = async () => {
    const cfg = getConfig();
    if (!cfg?.consent_given) return;
    try {
      ...
      if (tickets.length > 0) {
        ...
        for (const t of tickets) {
          ...
          enqueue(ev);
          if (t.date_heure > latest) latest = t.date_heure;
        }
...
export function ensureCommerceId(cfg: AgentConfig): AgentConfig {
  if (cfg.commerce_internal_id) return cfg;
