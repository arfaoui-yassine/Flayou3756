import type { DataSoukEvent } from '@datasouk/shared';
...
export async function syncPendingEvents(
  ...
  if (batch.length === 0) return { sent: 0, failed: 0, skipped: false };

  const events: DataSoukEvent[] = [];
  const ids: number[] = [];
  for (const row of batch) {
    try {
      events.push(JSON.parse(row.payload) as DataSoukEvent);
      ids.push(row.id);
    } catch (e) {
      log.error({ err: e, id: row.id }, 'bad queue payload');
    }
  }
  if (events.length === 0) return { sent: 0, failed: 0, skipped: false };
