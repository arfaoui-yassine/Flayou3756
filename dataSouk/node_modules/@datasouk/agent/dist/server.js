...
function replyInnovaFailure(reply, e, logLabel) {
    const msg = errMessage(e);
    log.error({ err: e }, logLabel);
    const looksLikePathOrDb = /SQLite:|fichier introuvable|dossier|ENOENT|ECONNREFUSED|Failed to connect|login failed/i.test(msg);
...
async function main() {
    ...
    const syncTimer = setInterval(() => {
        const c = getConfig();
        if (!c?.consent_given || !c.cloud_api_key)
            return;
        void syncPendingEvents(c.cloud_api_url, c.cloud_api_key, AGENT_VERSION).catch((e) => log.error({ err: e }, 'sync interval error'));
    }, 60_000);
